...
CKEDITOR.ENTER_P ? 'p' : 'div'; if (c) { switch (b) { case 'left':this.cssClassName = c[0]; break; case 'center':this.cssClassName = c[1]; break; case 'right':this.cssClassName = c[2]; break; case 'justify':this.cssClassName = c[3] } this.cssClassRegex = new RegExp('(?:^|\\s+)(?:' + c.join('|') + ')(?\x3d$|\\s)'); this.requiredContent = f + '(' + this.cssClassName + ')' } else this.requiredContent = f + '{text-align}'; this.allowedContent = {
    ...
    var c = a.editor; var b = c.createRange(); b.setStartBefore(a.data.node); b.setEndAfter(a.data.node); for (var f = new CKEDITOR.dom.walker(b), d; d = f.next();) {
    ...
    exec: function (a) {
      var c = a.getSelection(); var b = a.config.enterMode; if (c) {
        for (var f = c.createBookmarks(), d = c.getRanges(), e = this.cssClassName, h, g, k = a.config.useComputedState, k = void 0 === k || k, n = d.length - 1; n >= 0; n--) {
          for (h = d[n].createIterator(), h.enlargeBr = b != CKEDITOR.ENTER_BR; g = h.getNextParagraph(b == CKEDITOR.ENTER_P ? 'p' : 'div');) {
            if (!g.isReadOnly()) {
              var l = g.getName()
              var p; p = a.activeFilter.check(l + '{text-align}'); if ((l = a.activeFilter.check(l + '(' + e + ')')) || p) { g.removeAttribute('align'); g.removeStyle('text-align'); var m = e && (g.$.className = CKEDITOR.tools.ltrim(g.$.className.replace(this.cssClassRegex, ''))); var r = this.state == CKEDITOR.TRISTATE_OFF && (!k || q(g, !0) != this.value); e && l ? r ? g.addClass(e) : m || g.removeAttribute('class') : r && p && g.setStyle('text-align', this.value) }
            }
          }
    ...
    refresh: function (a, c) {
      ...
      var f = b.getName(); var d = b.equals(a.editable()); var f = this.cssClassName ? a.activeFilter.check(f + '(' + this.cssClassName + ')') : a.activeFilter.check(f + '{text-align}'); d && c.elements.length === 1 ? this.setState(CKEDITOR.TRISTATE_OFF) : !d && f ? this.setState(q(b, this.editor.config.useComputedState) == this.value ? CKEDITOR.TRISTATE_ON : CKEDITOR.TRISTATE_OFF) : this.setState(CKEDITOR.TRISTATE_DISABLED)
    }
