...
(function () {
  function f (a) {
    a = a.getElementsByTag('*'); for (var c = a.count(), b, d = 0; d < c; d++) {
      b = a.getItem(d), (function (a) {
        for (var b = 0; b < l.length; b++) {
          (function (b) {
            var d = a.getAttribute('on' + b); a.hasAttribute('on' + b) && (a.removeAttribute('on' + b), a.on(b, function (b) {
              var c = /(return\s*)?CKEDITOR\.tools\.callFunction\(([^)]+)\)/.exec(d); var k = c && c[1]; var e = c && c[2].split(','); var c = /return false;/.test(d); if (e) {
    ...
    onLoad: function () {
      CKEDITOR.env.air && (CKEDITOR.dom.document.prototype.write = CKEDITOR.tools.override(CKEDITOR.dom.document.prototype.write, function (a) {
        function c (b,
          ...
          if (this.getBody()) {
            var d = this; var f = this.getHead(); b = b.replace(/(<style[^>]*>)([\s\S]*?)<\/style>/gi, function (a, b, d) { c(f, 'style', b, d); return '' }); b = b.replace(/<base\b[^>]*\/>/i, function (b) { c(f, 'base', b); return '' }); b = b.replace(/<title>([\s\S]*)<\/title>/i, function (b, a) { d.$.title = a; return '' }); b = b.replace(/<head>([\s\S]*)<\/head>/i, function (b) {
              var a =
new CKEDITOR.dom.element('div', d); a.setHtml(b); a.moveChildren(f); return ''
        ...
        a = a.data; if (a._.panel) {
          var c = a._.panel._.panel; var b; (function () {
