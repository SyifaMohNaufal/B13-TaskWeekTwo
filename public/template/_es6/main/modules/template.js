...
export default class Template {
    ...
    if (self._lMain.length) {
      jQuery(window).on('resize.cb.main orientationchange.cb.main', e => {
        ...
        resizeTimeout = setTimeout(e => {
          const hWindow = jQuery(window).height()
          const hHeader = self._lHeader.outerHeight() || 0
          const hFooter = self._lFooter.outerHeight() || 0
  ...
  _uiHandleForms () {
    jQuery('.form-material.floating > .form-control').each((index, element) => {
      ...
      const parent = input.parent('.form-material')
  ...
  _uiHandleTheme () {
    ...
    const cookies = !!this._lPage.hasClass('enable-cookies')

    // If cookies are enabled
    if (cookies) {
      const themeName = Cookies.get('cbThemeName') || false
  ...
  _uiApiBlocks (block = false, mode = 'init') {
    ...
    const iconFullscreen = 'si si-size-fullscreen'
    const iconFullscreenActive = 'si si-size-actual'
    const iconContent = 'si si-arrow-up'
    const iconContentActive = 'si si-arrow-down'
