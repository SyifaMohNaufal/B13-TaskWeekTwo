...
export default class Helpers {
  ...
  static coreBootstrapCustomFileInput () {
    ...
    jQuery('[data-toggle="custom-file-input"]:not(.js-custom-file-input-enabled)').each((index, element) => {
      ...
      el.addClass('js-custom-file-input-enabled').on('change', e => {
        const fileName = (e.target.files.length > 1) ? e.target.files.length + ' ' + (el.data('lang-files') || 'Files') : e.target.files[0].name
  ...
  static coreScrollTo () {
    jQuery('[data-toggle="scroll-to"]:not(.js-scroll-to-enabled)').on('click.cb.helpers.core', e => {
      ...
      // Set variables
      const lHeader = jQuery('#page-header')
      const el = jQuery(e.currentTarget)
      const elTarget = el.data('target') || el.attr('href')
      const elSpeed = el.data('speed') || 1000
      const headerHeight = (lHeader.length && jQuery('#page-container').hasClass('page-header-fixed')) ? lHeader.outerHeight() : 0
  ...
  /*
     ...
     */
  static coreYearCopy () {
    const el = jQuery('.js-year-copy:not(.js-year-copy-enabled)')

    if (el.length > 0) {
      const date = new Date()
      const curYear = date.getFullYear()
      const baseYear = (el.html().length > 0) ? el.html() : curYear
  ...
  static coreAppearCountTo () {
    ...
    jQuery('[data-toggle="countTo"]:not(.js-count-to-enabled)').each((index, element) => {
      const el = jQuery(element)
      const elAfter = el.data('after')
      const elBefore = el.data('before')
    ...
    jQuery('[data-toggle="click-ripple"]:not(.js-click-ripple-enabled)').each((index, element) => {
      ...
      el.addClass('js-click-ripple-enabled')
        .css({
          ...
          const cssClass = 'click-ripple'; let ripple; let d; let x; let y
  ...
  static print () {
    ...
    const lPage = jQuery('#page-container')
    const pageCls = lPage.prop('class')
  ...
  static tableToolsCheckable () {
    ...
    jQuery('.js-table-checkable:not(.js-table-checkable-enabled)').each((index, element) => {
      ...
      jQuery('thead input:checkbox', table).on('click.cb.helpers', e => {
        const checkedStatus = jQuery(e.currentTarget).prop('checked')
      ...
      jQuery('tbody input:checkbox, tbody input + label', table).on('click.cb.helpers', e => {
        ...
        const checkedStatus = checkbox.prop('checked')
      ...
      jQuery('tbody > tr', table).on('click.cb.helpers', e => {
        if (e.target.type !== 'checkbox' &&
                        ...
                        !jQuery(e.target).parent('.custom-control').length &&
                        !jQuery(e.target).parent('label').length) {
          const checkbox = jQuery('input:checkbox', e.currentTarget)
          const checkedStatus = checkbox.prop('checked')
  ...
  static contentFilter () {
    ...
    jQuery('.js-filter:not(.js-filter-enabled)').each((index, element) => {
      const el = jQuery(element)
      const filterNav = jQuery('.nav-pills', el)
      const filterLinks = jQuery('a[data-category-link]', el)
      const filterItems = jQuery('[data-category]', el)
      const filterSpeed = el.data('speed') || 200
      ...
      if (el.data('numbers')) {
        filterLinks.each((index, element) => {
          const filterLink = jQuery(element)
          const filterCat = filterLink.data('category-link')
      ...
      filterLinks.on('click.cb.helpers', e => {
        const filterLink = jQuery(e.currentTarget)
        let filterCat
  ...
  static slimscroll () {
    ...
    jQuery('[data-toggle="slimscroll"]:not(.js-slimscroll-enabled)').each((index, element) => {
      ...
      el.addClass('js-slimscroll-enabled').slimScroll({
        ...
        alwaysVisible: !!el.data('always-visible'),
        railVisible: !!el.data('rail-visible'),
        railColor: el.data('rail-color') || '#999',
        railOpacity: el.data('rail-opacity') || 0.3
      })
    })
  }
